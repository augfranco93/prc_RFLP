...
import numpy as np
#Data
data = pd.read_csv("PCR-RFLP/16S_sequences/arb-silva.de_testprime_taxlist_966407.csv")
# data as a data frame
sequences = pd.DataFrame(data)

#Filtering for only BACTERIASP species. 
df =sequences.filter(["primaryAccession", "path", "organismName","length"])
df2 = df[df["path"].str.contains("BACTERIASP")]

# Dataframe to a csv file
df2.to_csv("PCR-RFLP/16S_sequences/filter_data.csv")

# Filter data
Obtain BACTERIASP species that were present 3 or more times at the PCR results
...
# Dataframe to a csv file
df_filter.to_csv("PCR-RFLP/16S_sequences/Data_filter_Counts.csv")
...
for i in (df_filter['primaryAccession']):
    ...
    for sequences in SeqIO.parse("16S_sequences/Filter_sequences.fasta","fasta"):
...
df_final.to_csv("PCR-RFLP/16S_sequences/PCR_results.csv") # DataFrame to a CSV
df_separated = df_final.explode('fragments_Size').reset_index(drop=True)
df_separated.to_csv("PCR-RFLP/16S_sequences/Final_PCR-Results.csv") # DataFrame to a CSV
